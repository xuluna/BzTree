...
#include "PMwCAS.h"
#include "bzerrno.h"
//#include "pmwcas/include/pmwcas.h"
#include "utils.h"
...
struct bz_node
{
	...
	void copy_value(Val * dst, const Val * src);
        ...
        /* 辅助函数 */
        /* 基本操作 */
        uint32_t binary_search(const Key *key, int size = 0,
                               uint64_t *meta_arr = nullptr);
        bool find_key_sorted(const Key *key, uint32_t &pos);
        bool find_key_unsorted(const Key *key, uint64_t status_rd,
                               uint32_t alloc_epoch, uint32_t &pos,
                               bool &recheck);
	...
	int rescan_unsorted(bz_tree<Key, TreeVal> * tree, uint32_t beg_pos, uint32_t rec_cnt, const Key * key, uint32_t total_size, uint32_t alloc_epoch);
        ...
        uint32_t copy_node_to(rel_ptr<bz_node<Key, Val>> dst,
                              uint64_t status_rd = 0);
        void fr_sort_meta();
	...
	int merge(bz_tree<Key, TreeVal> * tree, int child_id, rel_ptr<bz_node<Key, uint64_t>> parent, rel_ptr<uint64_t> grandpa_status, rel_ptr<uint64_t> grandpa_ptr);
        ...
        /* 执行叶节点的数据项操作 */
        template <typename TreeVal>
        int insert(bz_tree<Key, TreeVal> *tree, const Key *key, const Val *val,
                   uint32_t key_size, uint32_t total_size,
                   uint32_t alloc_epoch);
        template <typename TreeVal>
        int remove(bz_tree<Key, TreeVal> *tree, const Key *key);
        template <typename TreeVal>
        int update(bz_tree<Key, TreeVal> *tree, const Key *key, const Val *val,
                   uint32_t key_size, uint32_t total_size,
                   uint32_t alloc_epoch);
        template <typename TreeVal>
        int read(bz_tree<Key, TreeVal> *tree, const Key *key, Val *buffer,
...
/* 位操作函数集 BEGIN */
inline bool is_frozen(uint64_t status) { return 1 & (status >> 60); }
inline void set_frozen(uint64_t &s) { s |= (1ULL << 60); }
inline void unset_frozen(uint64_t &s) { s &= ~(1ULL << 60); }
...
inline uint32_t get_delete_size(uint64_t status) { return 0x3fffff & status; }
inline void set_delete_size(uint64_t &s, uint32_t new_delete_size) {
  s = (s & 0xffffffffffc0000) | new_delete_size;
}
static inline bool is_leaf(uint64_t length) { return !(1 & length); }
inline void set_non_leaf(uint64_t &length) { length |= 1; }
inline void set_leaf(uint64_t &length) { length &= (~1ULL); }
inline uint32_t get_node_size(uint64_t length) { return length >> 32; }
...
inline bool is_visiable(uint64_t meta) { return !(1 & (meta >> 60)); }
inline void set_visiable(uint64_t &meta) { meta &= 0xefffffffffffffff; }
inline void unset_visiable(uint64_t &meta) { meta |= ((uint64_t)1 << 60); }
inline uint32_t get_offset(uint64_t meta) { return 0xfffffff & (meta >> 32); }
inline void set_offset(uint64_t &meta, uint64_t new_offset) {
  meta = (meta & 0xf0000000ffffffff) | (new_offset << 32);
}
inline uint16_t get_key_length(uint64_t meta) { return 0xffff & (meta >> 16); }
inline void set_key_length(uint64_t &meta, uint32_t new_key_length) {
  meta = (meta & 0xffffffff0000ffff) | (new_key_length << 16);
}
inline uint32_t get_total_length(uint64_t meta) { return 0xffff & meta; }
...
bool bz_tree<Key, Val>::smo(bz_path_stack * path_stack, int & ret)
{
	...
	CONSOLIDATE_TAG:
		if (child_id < 0) {
                  ret = node->template consolidate<Val>(
                      this, rel_ptr<uint64_t>::null(),
                  ...
                  rel_ptr<bz_node<Key, uint64_t>> parent(
                      path_stack->get_node());
                  ret = node->template consolidate<Val>(
                      this, &parent->status_, parent->nth_val(child_id));
                  path_stack->push();
                }
        } else if (smo_type == BZ_SPLIT) {
          if (child_id < 0) {
            ret = node->template split<Val>(
                this, rel_ptr<bz_node<Key, uint64_t>>::null(),
            ...
            if (grand_id < 0) {
              ret = node->template split<Val>(this, parent,
                                              rel_ptr<uint64_t>::null(),
                                              rel_ptr<uint64_t>::null());
            } else {
              path_stack->pop();
              rel_ptr<bz_node<Key, uint64_t>> grandpa(path_stack->get_node());
              ret = node->template split<Val>(this, parent, &grandpa->status_,
                                              grandpa->nth_val(grand_id));
              path_stack->push();
            }
            path_stack->push();
          }
          ...
          int grand_id = path_stack->get_child_id();
          if (grand_id < 0) {
            ret = node->template merge<Val>(this, child_id, parent,
                                            rel_ptr<uint64_t>::null(),
                                            rel_ptr<uint64_t>::null());
          } else {
            path_stack->pop();
...
rel_ptr<rel_ptr<bz_node<Key, NType>>> bz_tree<Key, Val>::alloc_node(mdesc_t mdesc, int magic)
{
	...
	return new_node_ptr;
        ...
        //原子分配空间
        TX_BEGIN(pop_) {
                pmemobj_tx_add_range_direct(new_node_ptr.abs(),
        sizeof(uint64_t)); *new_node_ptr = pmemobj_tx_alloc(NODE_ALLOC_SIZE,
        TOID_TYPE_NUM(struct bz_node_fake)); } TX_END;
	...
	rel_ptr<uint64_t> grandpa_status, rel_ptr<uint64_t> grandpa_ptr)
{
	...
	if (mdesc.is_null())
        ...
        while (true) {
          ...
          if (!forbids[1] && !sibling_type &&
              (uint32_t)child_id + 1 < child_max) {
            //判断右兄弟
            sibling = parent->nth_child(child_id + 1);
            status_sibling = pmwcas_read(&sibling->status_);
            if (!is_frozen(status_sibling)) {
          ...
          pmwcas_add(mdesc, &sibling->status_, status_sibling_new,
                     status_sibling, NOCAS_EXECUTE_ON_FAILED);
        ...
        //删除根节点
        if (parent.is_null()) {
          pmwcas_add(mdesc, &tree->root_,
                     rel_ptr<bz_node<Key, Val>>(this).rel(), 0,
        ...
        if (sibling_type) {
          /* 初始化N' */
          new_node_ptr = tree->template alloc_node<Val>(mdesc, 0);
          rel_ptr<bz_node<Key, Val>> new_node = *new_node_ptr;
		...
		|| new_node_ptr.is_null())
	{
          ...
          /* 初始化P' BEGIN */
          new_parent_ptr = tree->template alloc_node<uint64_t>(mdesc, 1);
          ...
          if (grandpa_ptr.is_null()) {
            ...
            pmwcas_add(mdesc, grandpa_status, status_grandpa_rd,
                       status_grandpa_rd);
          }
        } else {
          //删除父亲节点
          ...
          if (grandpa_ptr.is_null()) {
            pmwcas_add(mdesc, &tree->root_, parent.rel(), new_node_ptr->rel(),
                       RELEASE_EXP_ON_SUCCESS);
          } else {
            pmwcas_add(mdesc, grandpa_ptr, parent.rel(), new_node_ptr->rel(),
                       RELEASE_EXP_ON_SUCCESS);
            uint64_t status_grandpa_rd = pmwcas_read(grandpa_status.abs());
            if (is_frozen(status_grandpa_rd)) {
              ret = EFROZEN;
	...
	rel_ptr<uint64_t> grandpa_status, rel_ptr<uint64_t> grandpa_ptr)
{
	...
	print_log("SPLIT_BEGIN");
        ...
        rel_ptr<rel_ptr<bz_node<Key, Val>>> new_left_ptr =
            tree->template alloc_node<Val>(mdesc, 0);
        ...
        rel_ptr<rel_ptr<bz_node<Key, Val>>> new_right_ptr =
            tree->template alloc_node<Val>(mdesc, 1);
        rel_ptr<bz_node<Key, Val>> new_right = *new_right_ptr;

        rel_ptr<rel_ptr<bz_node<Key, uint64_t>>> new_parent_ptr =
            tree->template alloc_node<uint64_t>(mdesc, 2);
        rel_ptr<bz_node<Key, uint64_t>> new_parent = *new_parent_ptr;
        ...
        //拷贝meta到new_right
        memcpy(new_right->rec_meta_arr(),
               new_left->rec_meta_arr() + left_rec_cnt,
	...
	if (!parent.is_null()) {
          ...
          uint32_t new_parent_rec_cnt =
              parent->copy_node_to(new_parent, status_parent_rd);
        ...
        if (!grandpa_ptr.is_null()) {
          ...
          pmwcas_add(mdesc, &parent->status_, status_parent_rd,
                     status_parent_new);
          ...
          pmwcas_add(mdesc, grandpa_status, status_grandpa_rd,
                     status_grandpa_rd);
          pmwcas_add(mdesc, this_node_addr, 0, 0,
                     NOCAS_RELEASE_ADDR_ON_SUCCESS);
	...
	rel_ptr<uint64_t> parent_status, rel_ptr<uint64_t> parent_ptr)
{
	...
	print_log("CONSOLIDATE_BEGIN");
        ...
        //初始化节点内容为0
        rel_ptr<rel_ptr<bz_node<Key, Val>>> node_ptr =
            tree->template alloc_node<Val>(mdesc);
...
inline void bz_node<Key, Val>::fr_remove_meta(int pos)
{
	...
	memmove(parent_meta_arr + pos, parent_meta_arr + pos + 1, (rec_cnt - 1 - pos) * sizeof(uint64_t));
        ...
        set_delete_size(status_,
                        dele_sz + get_total_length(parent_meta_arr[pos]));
}
...
int bz_node<Key, Val>::fr_root_init(const Key * K, uint64_t left, uint32_t key_sz, uint64_t right)
{
	...
	copy_data(left_key_offset, K, &left, key_sz, tot_sz);
        ...
        meta_arr[1] = meta_vis_off_klen_tlen(
            0, true, right_key_offset, sizeof(uint64_t), sizeof(uint64_t) * 2);
...
uint32_t bz_node<Key, Val>::copy_sort_meta_to(rel_ptr<bz_node<Key, Val>> dst)
{
  ...
  std::sort(new_meta_arr, new_meta_arr + new_rec_cnt,
            std::bind(&bz_node<Key, Val>::key_cmp_meta, this,
                      std::placeholders::_1, std::placeholders::_2));
  //计算有效meta数目
  return this->binary_search(nullptr, new_rec_cnt, new_meta_arr);
  //将剩余部分置空
  // memset(new_meta_arr + new_rec_cnt, 0, (rec_cnt - new_rec_cnt) *
  // sizeof(uint64_t)); return new_rec_cnt;
}
...
int bz_node<Key, Val>::insert(bz_tree<Key, TreeVal> * tree, const Key * key, const Val * val, uint32_t key_size, uint32_t total_size, uint32_t alloc_epoch)
{
	...
	uint32_t rec_cnt, blk_sz;
        ...
        while (true) {
          ...
          /* 可容纳 */
          if (blk_sz + total_size + sizeof(uint64_t) * (1 + rec_cnt) +
                  sizeof(*this) >
              node_sz)
            return EALLOCSIZE;
          ...
          /* 2-word PMwCAS */
          int cas_res =
              tree->pack_pmwcas({{&status_, status_rd, status_new},
...
int bz_node<Key, Val>::remove(bz_tree<Key, TreeVal> * tree, const Key * key)
{
	...
	if (is_frozen(status_rd))
        ...
        bool useless;
        if (!find_key_sorted(key, pos) &&
            !find_key_unsorted(key, status_rd, 0, pos, useless))
          return ENOTFOUND;
        ...
        while (true)
	{
		...
		if (!is_visiable(meta_rd)) {
                ...
                /* 增加delete size */
                uint64_t status_new =
                    status_del(status_rd, get_total_length(meta_rd));
...
int bz_node<Key, Val>::update(bz_tree<Key, TreeVal> * tree, const Key * key, const Val * val, uint32_t key_size, uint32_t total_size, uint32_t alloc_epoch)
{
	...
	while (true)
	{
		...
		if (!is_visiable(meta_del)) {
                ...
                uint64_t status_new =
                    status_del(status_rd, get_total_length(meta_del));
...
void bz_node<Key, Val>::copy_key(Key * dst, const Key * src) {
	if (*(uint64_t*)src == BZ_KEY_MAX)
		*(uint64_t*)dst = BZ_KEY_MAX;
          ...
          strncpy((char *)dst, (char *)src, strlen((char *)src) + 1);
        else
		*dst = *src;
}
template<typename Key, typename Val>
